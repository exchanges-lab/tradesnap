...
async fn main() -> Result<()> {
    ...
    println!(
        "Loaded config: symbol=BYBIT:BTCUSDT.P, chart_page_id={}",
    ...
    match tab.get_cookies() {
        Ok(cookies) => {
            ...
            for c in cookies {
                println!(
                    "  Cookie: name={}, domain={}, path={}, secure={}, http_only={}",
                    c.name, c.domain, c.path, c.secure, c.http_only
                );
            }
        }
        Err(e) => {
    ...
    copy_link_el
        .click()
        .map_err(|e| anyhow!("Failed to click 'Copy link' element: {}", e))?;
